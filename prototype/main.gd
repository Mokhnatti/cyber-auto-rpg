...
func _new_gear(cls: int) -> Dictionary:
	...
	var gear := {
		...
		"weapon": {wkey: {"vid": wfirst["id"], "rarity": 1, "lvl": 1, "rolls": [_roll_stat(wfirst["stat"])]}},
	}
...
func _load() -> void:
	...
	for i in min(hs.size(), heroes.size()):
		...
		if not heroes[i]["gear"].has("weapon") or not heroes[i]["equip"].has("weapon"):
			...
			heroes[i]["gear"]["weapon"] = {wkey: {"vid": wf["id"], "rarity": 1, "lvl": max(1, int(s.get("wlvl", 1))), "rolls": [_roll_stat(wf["stat"])]}}
...
func _passive_rate() -> float:
	# ПАССИВ/с = (база + надбавка от глубины) × аугмент золота → растёт со стадией, не залипает на 2
	return (gold_ps + float(max(stage, best_stage)) * 1.5) * aug_gold
