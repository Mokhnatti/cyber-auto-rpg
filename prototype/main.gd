...
func _process(delta: float) -> void:
	...
	if save_t <= 0.0:
		save_t = 10.0
		_save()
		print("TTSTATE stage=%d sub=%d boss=%d best=%d cores=%d scrap=%d gold=%d slots=%d/%d augs=%d" % [stage, sub, (1 if in_boss else 0), best_stage, cores, scrap, int(gold), equipped_augs.size(), _slot_total(), aug_lvl.size()])
	if bot:
		_bot_tick(delta)
	if phase == "dead":
		return
