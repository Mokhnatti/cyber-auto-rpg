...
const VERSION := "1.9.32" # версия билда (показывается в игре: тестер видит совпадает ли с последней → надо ли обновиться). Бампить КАЖДЫЙ деплой.
var nick := ""
...
func _reset() -> void:
	...
	cores = 0
	cores_peak = 0.0
	diamonds = 50; x3_unlocked = false; x2_until = 0.0; gacha_pity = 0; offline_cap_lvl = 0; last_discovered = ""; ad_boosts = {}; clan_boosts = {}
...
func _load() -> void:
	...
	diamonds = int(d.get("diamonds", 50)); x3_unlocked = bool(d.get("x3_unlocked", false)); x2_until = float(d.get("x2_until", 0.0))
...
func _deal(hh: Dictionary, e: Dictionary, d: float, is_crit := false) -> void:
	...
	if e["hp"] <= 0 and e["alive"]:
		e["alive"] = false
		var kg: float = (50.0 if e.get("boss", false) else 5.0) * pow(GOLD_PER_STAGE, stage - 1) * aug_gold * _ad_mult("gold") * _clan_boost_mult("gold") * _event_gold_mult()   # ×бусты золота ×ивент
		gold += kg
		_stat_add("gold", kg)
...
func _passive_rate() -> float:
	...
	return (gold_ps + float(max(stage, best_stage)) * 1.5) * aug_gold * _ad_mult("gold") * _clan_boost_mult("gold") * _event_gold_mult()
