...
func _recalc_hero(hh: Dictionary) -> void:
	...
	hh["dmg"] = int(round(min(base_dmg * lv * milestone * aug_dmg * _ad_mult("dmg"), STAT_CAP)))   # ×реклама-буст урона
...
func _save() -> void:
	...
	var d := {
		...
		"best_stage": best_stage, "scrap": scrap, "cores": cores, "cores_peak": cores_peak, "diamonds": diamonds, "x3_unlocked": x3_unlocked, "gacha_pity": gacha_pity, "ad_boosts": ad_boosts,
