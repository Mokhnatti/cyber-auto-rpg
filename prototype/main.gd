...
const VERSION := "1.9.126" # версия билда (показывается в игре: тестер видит совпадает ли с последней → надо ли обновиться). Бампить КАЖДЫЙ деплой.
...
# ===== ENDLESS (Бесконечная башня) — ретеншн-режим: серия волн без престижа, цель «зайти выше» =====
const ENDLESS_GATE_STAGE := 30      # открывается только со стадии 30+ (не грузим новичка — как сингулярность гейтнута)
const ENDLESS_ANCHOR_OFFSET := 8    # этаж-1 = (best_stage - 8) по сложности → комфортный старт, потом ритмично ужесточается
const ENDLESS_STEP := 0.5           # на сколько «стадий-эквивалента» тяжелеет каждый этаж (0.5 → ~16 этажей до личной стены)
const ENDLESS_SCRAP_PER_FLOOR := 18 # награда: лом ♻ за достигнутый этаж (пропорц.)
...
const BOSS_HP_CYCLE := [7.0, 9.0, 11.0, 13.0, 20.0]   # множитель HP босса (фидбэк Насти: боссы дохли мгновенно — задрано ~×2, каждый 5-й = milestone-стена). Тюнить ботами
...
var fb_id_token := ""       # idToken анон-сессии — шлётся в _fb_rest как ?auth (крит безопасности: без него БД открыта всем)
var fb_refresh_token := ""  # refreshToken → обновляем idToken каждые ~45 мин (idToken живёт 1ч), иначе долгие сессии → 401 после строгих правил RTDB
var fb_ready := false
var fb_t := 0.0
...
# ⚙️ КЛАН-ТЕХНОЛОГИИ — 3 древа × 5 ячеек (дизайн Дианы+мой). Ключ ячейки = "<tree>.<key>".
const CLAN_TREES := {
	"war": {"name": "🗡 ВОЙНА", "name_en": "🗡 WAR", "col": "#ff4d5e", "cells": [
		{"key": "dmg",   "icon": "💥", "name": "Урон отряда",   "name_en": "Team Damage",  "per": 0.02, "unit": "% урона",  "unit_en": "% dmg",  "max": 20},
		{"key": "atk",   "icon": "⚡", "name": "Скор. атаки",   "name_en": "Attack Speed", "per": 0.02, "unit": "% скор.",  "unit_en": "% atk",  "max": 20},
		{"key": "crit",  "icon": "🎯", "name": "Шанс крита",    "name_en": "Crit Chance",  "per": 0.01, "unit": "% крит",  "unit_en": "% crit", "max": 15},
		{"key": "critx", "icon": "🔥", "name": "Крит-урон",     "name_en": "Crit Damage",  "per": 0.03, "unit": "× крит",  "unit_en": "x crit", "max": 15},
		{"key": "hp",    "icon": "🛡", "name": "HP отряда",     "name_en": "Team HP",      "per": 0.02, "unit": "% HP",    "unit_en": "% HP",   "max": 20},
	]},
	"econ": {"name": "💰 СИНДИКАТ", "name_en": "💰 SYNDICATE", "col": "#ffd24a", "cells": [
		{"key": "gold",    "icon": "💰", "name": "Золото",        "name_en": "Gold",         "per": 0.04, "unit": "% золота", "unit_en": "% gold",  "max": 20},
		{"key": "scrap",   "icon": "♻", "name": "Лом",           "name_en": "Scrap",        "per": 0.04, "unit": "% лома",   "unit_en": "% scrap", "max": 20},
		{"key": "loot",    "icon": "🔫", "name": "Дроп пушек",    "name_en": "Weapon Drop",  "per": 0.03, "unit": "% дропа",  "unit_en": "% drop",  "max": 15},
		{"key": "offline", "icon": "🌙", "name": "Оффлайн-доход", "name_en": "Offline Gain", "per": 0.04, "unit": "% офлайн", "unit_en": "% offline", "max": 15},
		{"key": "upg",     "icon": "📉", "name": "Скидка прокачки","name_en": "Upgrade Disc.","per": 0.015,"unit": "% скидка", "unit_en": "% off",   "max": 15},
	]},
	"net": {"name": "⚙️ СЕТЬ", "name_en": "⚙️ NET", "col": "#7adfff", "cells": [
		{"key": "ultcd",   "icon": "🔋", "name": "КД ульт",       "name_en": "Ult Cooldown", "per": 0.015,"unit": "% КД",     "unit_en": "% CD",    "max": 15},
		{"key": "cboss",   "icon": "🩸", "name": "Урон клан-боссу","name_en": "Clan Boss Dmg","per": 0.05, "unit": "% босс",   "unit_en": "% boss",  "max": 15},
...
const GEM_DON_BASE := 20      # 💎 базовая цена вклада алмазами (пачками по 5: 5×20 → 5×40 …)
const CLAN_DON_CRIT := 0.15   # 🎲 шанс крит-вклада ×2 очков (реф Дианы)
...
var gem_confirm_skip_day := -1  # день, когда галка «не показывать подтверждение» активна
...
const WEEKLY_EVENTS := [
	...
	{"id": "loot",     "icon": "🎁"},   # +1 к полу редкости выпадающего лута
	{"id": "prestige", "icon": "🧬"},   # +25% ядер за престиж
]
# локализация ивентов (имя + описание) — отдельно от основного loc-словаря для компактности
const EVENT_LOC := {
	"gold":     {"name": "🪙 Золотой уикенд", "name_en": "🪙 Golden Weekend",
	             "desc": "Враги роняют ×2 золота, и пассивный доход тоже ×2. Лучшее время прокачивать бойцов!",
	             "desc_en": "Enemies drop ×2 gold and passive income is ×2 too. Best time to level your squad!"},
	"gacha":    {"name": "💎 Гача-неделя", "name_en": "💎 Gacha Week",
	             "desc": "Все гача-пуллы дешевле на 20%. Лови момент для круток!",
	             "desc_en": "All gacha pulls are 20% cheaper. Grab the moment to roll!"},
	"loot":     {"name": "🎁 Лут-неделя", "name_en": "🎁 Loot Week",
	             "desc": "Дроп немного лучше: пол редкости выпадающего лута поднят на ступень.",
	             "desc_en": "Drops are a bit better: the rarity floor of dropped loot is raised by one tier."},
	"prestige": {"name": "🧬 Престиж-неделя", "name_en": "🧬 Prestige Week",
...
const TR := {
	...
	"tut_s2": {"ru": "⚡ Тапни ульту бойца, когда готова", "en": "⚡ Tap a fighter's ult when ready"},
	"tut_s3": {"ru": "⚔ Бей волны — собирай лут", "en": "⚔ Fight the waves — collect loot"},
	"tut_s4": {"ru": "🎁 Первый лут! Надень его — 🦾 Экипировка", "en": "🎁 First loot! Equip it — 🦾 Gear"},
	...
	"min_short": {"ru": "м", "en": "m"},
	"day_short": {"ru": "д", "en": "d"},
	# недельный ивент (LiveOps)
	"we_banner": {"ru": "📅 Ивент недели", "en": "📅 Weekly Event"},
	"we_left":   {"ru": "⏳ До конца недели: %s", "en": "⏳ Ends in: %s"},
	# панель ЭКИПИРОВКА
	...
	"cl_tech_hint": {"ru": "Вкладывайте золото и алмазы → клан копит опыт → лидер качает перки для ВСЕХ навсегда.", "en": "Donate gold and diamonds → the clan gains XP → the leader levels perks for ALL members forever."},
	"cl_tech_hdr": {"ru": "🏦 Пул клана: %s очков", "en": "🏦 Clan pool: %s points"},
	"cl_res_active": {"ru": "⏳ Изучается: %s  (%d:%02d)", "en": "⏳ Researching: %s  (%d:%02d)"},
	"cl_res_idle": {"ru": "Изучение не идёт — офицер может запустить", "en": "No research active — an officer can start one"},
	"cl_res_btn": {"ru": "▶ Изучить\n%s очков", "en": "▶ Research\n%s pts"},
	"cl_res_wait": {"ru": "⏳ %d:%02d", "en": "⏳ %d:%02d"},
	"cl_res_officer": {"ru": "Изучение запускает лидер 👑 или зам", "en": "Only leader 👑 or deputy can start research"},
	...
	"cl_don_nogem": {"ru": "Мало алмазов для вложения", "en": "Not enough diamonds to donate"},
	"cl_don_crit": {"ru": "🎲 КРИТ! +%s клан-очков ×2!", "en": "🎲 CRIT! +%s clan points ×2!"},
	"cl_gem_confirm": {"ru": "Потратить %d 💎 на вклад в клан?", "en": "Spend %d 💎 on a clan donation?"},
	"cl_gem_skip": {"ru": "Не спрашивать сегодня", "en": "Don't ask again today"},
	"cl_gem_yes": {"ru": "💎 %d — Вложить", "en": "💎 %d — Donate"},
	# чат клана
	...
	"offline_ad_x2":     {"ru": "📺 ×2 ЗОЛОТО (реклама)", "en": "📺 ×2 GOLD (ad)"},
	"offline_ad_pop":    {"ru": "📺 Оффлайн-золото удвоено! +%s", "en": "📺 Offline gold doubled! +%s"},
	...
	"m_endless":         {"ru": "🗼  Бесконечная башня", "en": "🗼  Endless Tower"},
	"endless_leave":     {"ru": "🚪  Покинуть башню", "en": "🚪  Leave the Tower"},
	"endless_enter":     {"ru": "🗼 БЕСКОНЕЧНАЯ БАШНЯ\nЗайди как можно ВЫШЕ!", "en": "🗼 ENDLESS TOWER\nClimb as HIGH as you can!"},
	"endless_locked":    {"ru": "🔒 Башня откроется со стадии %d", "en": "🔒 Tower unlocks at stage %d"},
	"endless_floor":     {"ru": "Этаж", "en": "Floor"},
	"endless_floor_up":  {"ru": "🗼 Этаж %d пройден!", "en": "🗼 Floor %d cleared!"},
	"endless_result":    {"ru": "☠ Башня: этаж %d · +%d♻", "en": "☠ Tower: floor %d · +%d♻"},
	"endless_record":    {"ru": "РЕКОРД! Этаж", "en": "RECORD! Floor"},
	...
	"vip_shop_active":   {"ru": "👑 КИБЕР-ПРОПУСК активен: %dд %dч (продлить)", "en": "👑 CYBER-PASS active: %dd %dh (extend)"},
	"vip_perks":         {"ru": "×2 всё золото · бесплатная x2-скорость · +%d💎/день", "en": "×2 all gold · free x2 speed · +%d💎/day"},
	"vip_buy_pop":       {"ru": "👑 Кибер-пропуск активен на %d дней!", "en": "👑 Cyber-Pass active for %d days!"},
	"vip_daily_pop":     {"ru": "👑 VIP-бонус: +%d💎", "en": "👑 VIP bonus: +%d💎"},
	...
	"firstbuy_offer_t":  {"ru": "⚡ БЫСТРЫЙ СТАРТ — 0.99$", "en": "⚡ FAST START — $0.99"},
	"firstbuy_offer_sub":{"ru": "Разовое предложение · только 24 часа", "en": "One-time offer · 24 hours only"},
	"firstbuy_perks":    {"ru": "🔫 Эпик-оружие первому бойцу\n⏩⏩ 24 ч ×2 скорость\n💎 +%d алмазов", "en": "🔫 Epic weapon for your 1st fighter\n⏩⏩ 24 h ×2 speed\n💎 +%d diamonds"},
	"firstbuy_btn":      {"ru": "⚡ ЗАБРАТЬ ЗА 0.99$", "en": "⚡ GET FOR $0.99"},
	"firstbuy_later":    {"ru": "Может позже", "en": "Maybe later"},
	"firstbuy_pop":      {"ru": "⚡ Быстрый старт активирован!", "en": "⚡ Fast start activated!"},
	"firstbuy_tile":     {"ru": "⚡ БЫСТРЫЙ СТАРТ", "en": "⚡ FAST START"},
	"firstbuy_timer":    {"ru": "⏳ осталось %s", "en": "⏳ %s left"},
	...
	"shop_val_single":   {"ru": "≈ %d круток", "en": "≈ %d pulls"},
	"shop_val_x10":      {"ru": "≈ %d ×10-призывов", "en": "≈ %d ×10 pulls"},
	"shop_val_guar":     {"ru": "≈ %d ×10 · гарант эпик ★", "en": "≈ %d ×10 · epic guaranteed ★"},
	...
	"shop_free_daily_wait":{"ru": "⏳ завтра", "en": "⏳ tomorrow"},
	"shop_weekly_pass":  {"ru": "СЕЗОННЫЙ ПРОПУСК\nнаграды каждую неделю", "en": "SEASON PASS\nweekly rewards"},
	...
	"reactor_desc":      {"ru": "Каждый уровень = +2 ч к окну офлайн-дохода.\nОтряд копит золото дольше, пока тебя нет.", "en": "Each level = +2 h to the offline-income window.\nYour squad banks gold longer while you're away."},
	"reactor_cur":       {"ru": "Окно офлайн-дохода: %d ч  (ур. %d/%d)", "en": "Offline window: %d h  (lvl %d/%d)"},
	"reactor_buy":       {"ru": "⬆ +2 ч  —  %d 💎", "en": "⬆ +2 h  —  %d 💎"},
	...
	"gacha_odds_title":   {"ru": "🎲 ШАНСЫ ДРОПА", "en": "🎲 DROP RATES"},
	"gacha_odds_rates_h": {"ru": "Базовый шанс за каждый пулл:", "en": "Base chance per single pull:"},
	"gacha_odds_pity_h":  {"ru": "Гарант (pity):", "en": "Guarantee (pity):"},
...
func _fb_on_auth(code: int, body: String) -> void:
	...
	fb_refresh_token = str(j.get("refreshToken", ""))
	fb_id = "#%06d" % (abs(hash(fb_uid)) % 1000000)
	fb_ready = true
	if fb_refresh_token != "" and not has_node("FbRefreshTimer"):   # авто-обновление idToken каждые 45 мин (idToken живёт 1ч) — крит: иначе долгие сессии → 401
		var _rt := Timer.new(); _rt.name = "FbRefreshTimer"; _rt.wait_time = 2700.0; _rt.autostart = true
		_rt.timeout.connect(_fb_refresh); add_child(_rt)
	print("FB ready uid=%s id=%s" % [fb_uid, fb_id])
	_fb_write_profile()

# обновление idToken по refreshToken (securetoken) — сохраняет тот же uid, продлевает сессию (крит: без этого долгие сессии → 401 после строгих правил RTDB)
func _fb_refresh() -> void:
	if bot or fb_refresh_token == "": return
	var http := HTTPRequest.new(); http.accept_gzip = false; add_child(http)
	http.request_completed.connect(func(_r, code, _h, data):
		if http.is_inside_tree(): http.queue_free()
		if int(code) != 200: return
		var j = JSON.parse_string(data.get_string_from_utf8())
		if typeof(j) == TYPE_DICTIONARY:
			fb_id_token = str(j.get("id_token", fb_id_token))
			fb_refresh_token = str(j.get("refresh_token", fb_refresh_token)))
	var headers := PackedStringArray(["Content-Type: application/x-www-form-urlencoded"])
	http.request("https://securetoken.googleapis.com/v1/token?key=" + FB_API_KEY, headers, HTTPClient.METHOD_POST, "grant_type=refresh_token&refresh_token=" + fb_refresh_token)
...
func _qa_poll() -> void:
	...
	if cmd.begins_with("shoptab_"):   # QA: открыть магазин на конкретной вкладке (скрины)
		shop_tab = cmd.substr(8); _open_shop(); return
	if cmd == "firstbuy":   # QA: активировать окно первой покупки + открыть магазин (скрин баннера)
		firstbuy_bought = false; firstbuy_until = _qa_now() + FIRSTBUY_WINDOW; _open_shop(); return
	if cmd == "fboffer":   # QA: попап первой покупки
		firstbuy_bought = false; firstbuy_until = _qa_now() + FIRSTBUY_WINDOW; _show_firstbuy_offer(); return
	...
	if cmd == "clantech":   # QA: экран клан-технологий с мок-данными (без Firebase)
		player_clan = "QATEST"; clan_xp = 4200000; clan_leader_uid = fb_uid; clan_tree = {"war": {"dmg": 5, "atk": 3, "crit": 2}, "econ": {"gold": 4, "loot": 1}, "net": {"cboss": 2}}; clan_research = {}; _open_clan_tech(); return
	...
	var m := {
		...
		"login": "_show_daily",
		"endless": "_endless_start", "endless_leave": "_endless_end",
		"event": "_open_event_popup",
	}
...
func _fb_rest(method: int, path: String, body: String, cb: Callable = Callable()) -> void:
	...
	var headers := PackedStringArray(["Content-Type: application/json"])
	# АВТОРИЗАЦИЯ (крит безопасности): шлём idToken анон-сессии → строгие правила RTDB пускают только authed. Без ?auth БД была открыта всем (утечка/накрутка).
	var url := FB_DB_URL + path + ".json"
	if fb_id_token != "": url += "?auth=" + fb_id_token
...
func _clan_boss_attack() -> void:
	...
	var hit: int = max(1, int(min(power_peak * randf_range(3.0, 5.0) * (1.0 + _cbon("net", "cboss")), 9.0e18)))   # ×клан урон по боссу
	boss_my_dmg += hit
	_save()
...
# ⚙️ КЛАН-ТЕХНОЛОГИИ ---
# --- ячейки древ ---
func _cell_def(tree: String, key: String) -> Dictionary:
	for c in CLAN_TREES.get(tree, {}).get("cells", []):
		if str(c["key"]) == key: return c
	return {}

func _cell_lvl(tree: String, key: String) -> int:
	return int(clan_tree.get(tree, {}).get(key, 0))

# стоимость изучения ур. lvl→lvl+1 (дорого — месяцами). Кламп int64.
func _cell_cost(lvl: int) -> int:
	return int(min(300000.0 * pow(1.7, float(lvl)), 9.0e15))

# длительность изучения (лидер запускает, ждём): 30мин × (lvl+1)
func _cell_research_secs(lvl: int) -> int:
	return 1800 * (lvl + 1)
...
func _cbon(tree: String, key: String) -> float:
	...
	return _cell_lvl(tree, key) * float(d["per"])

# совместимость: старые вызовы _clan_perk_mult("dmg"/"atk"/"gold")
func _clan_perk_mult(key: String) -> float:
	match key:
		"dmg": return 1.0 + _cbon("war", "dmg")
		"atk": return 1.0 + _cbon("war", "atk")
		"gold": return 1.0 + _cbon("econ", "gold")
	return 1.0

# роль игрока в клане
func _clan_is_leader() -> bool: return player_clan == "QATEST" or (clan_leader_uid == fb_uid and fb_uid != "")
func _clan_is_officer() -> bool: return _clan_is_leader() or bool(clan_deputies.get(fb_uid, false))
...
func _gem_cost() -> int:
	_don_day_check()
	return GEM_DON_BASE * (1 + cd_gem / 5)   # ПАЧКАМИ (реф Дианы): 5×20 → 5×40 → 5×60 …

# 🎲 крит вклада (реф Дианы): шанс ×2 очков. Возвращает [итог_очков, было_ли_крит].
func _don_crit(base: int) -> Array:
	if randf() < CLAN_DON_CRIT: return [base * 2, true]
	return [base, false]

func _don_reward_popup(pts: int, crit: bool, col: Color) -> void:
...
func _donate_gem_confirm(tree: String, key: String, on_done: Callable) -> void:
	...
	var sb := StyleBoxFlat.new(); sb.bg_color = Color(0.06, 0.10, 0.16, 0.99); sb.set_corner_radius_all(14); sb.border_color = Color("#7adfff"); sb.set_border_width_all(2); sb.set_content_margin_all(18)
	card.add_theme_stylebox_override("panel", sb); card.position = Vector2(W * 0.5 - 175, 350); card.custom_minimum_size = Vector2(350, 0); dlg.add_child(card)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 12); card.add_child(v)
	v.add_child(_lbl(_t("cl_gem_confirm") % _gem_cost(), 16, Color("#cfe6ff"), HORIZONTAL_ALIGNMENT_CENTER))
	var skip := CheckBox.new(); skip.text = _t("cl_gem_skip"); skip.add_theme_font_size_override("font_size", 13); v.add_child(skip)
	var ok := Button.new(); ok.text = _t("cl_gem_yes") % _gem_cost(); ok.custom_minimum_size = Vector2(0, 48); ok.add_theme_color_override("font_color", Color("#7adfff")); v.add_child(ok)
	var no := Button.new(); no.text = _t("close_x"); no.custom_minimum_size = Vector2(0, 40); v.add_child(no)
...
# подтянуть весь клан-узел в кэш (пул/древо/лидер/замы/изучение) + применить завершённое изучение
func _clan_sync_tech(on_done: Callable = Callable()) -> void:
	if player_clan == "" or player_clan == "QATEST" or not fb_ready:
	...
	_fb_rest(HTTPClient.METHOD_GET, "/clans/%s" % player_clan, "", func(_c, d):
		...
		if typeof(clan_research) == TYPE_DICTIONARY and clan_research.has("cell") and _qa_now() >= float(clan_research.get("ends", 0)):
			var parts := str(clan_research["cell"]).split(".")
			if parts.size() == 2:
				var tocall := int(clan_research.get("toLvl", 1))
				if not clan_tree.has(parts[0]): clan_tree[parts[0]] = {}
				clan_tree[parts[0]][parts[1]] = tocall
				_fb_rest(HTTPClient.METHOD_PUT, "/clans/%s/tree/%s/%s" % [player_clan, parts[0], parts[1]], str(tocall))
				_fb_rest(HTTPClient.METHOD_DELETE, "/clans/%s/research" % player_clan, "")
				clan_research = {}
		_recalc_auras()
		if on_done.is_valid(): on_done.call())
...
func _clan_start_research(tree: String, key: String) -> void:
	...
	var lvl := _cell_lvl(tree, key)
	var d := _cell_def(tree, key)
	if d.is_empty() or lvl >= int(d["max"]): return
	var cost := _cell_cost(lvl)
	if clan_xp < cost: _popup_center(_t("cl_res_nopool") % _gsep(cost), Color("#ff5050"), 2.0); return
	var newpool := clan_xp - cost
	clan_xp = newpool
	var ends := _qa_now() + float(_cell_research_secs(lvl))
	clan_research = {"cell": tree + "." + key, "ends": ends, "toLvl": lvl + 1}
	_fb_rest(HTTPClient.METHOD_PUT, "/clans/%s/xp" % player_clan, str(newpool))
	_fb_rest(HTTPClient.METHOD_PUT, "/clans/%s/research" % player_clan, JSON.stringify(clan_research))
	_popup_center(_t("cl_res_started") % _tloc(d, "name"), Color("#7ee08a"), 2.0)
...
func _open_node_detail(tree: String, key: String) -> void:
	...
	var bgold := Button.new(); bgold.custom_minimum_size = Vector2(0, 62); bgold.size_flags_horizontal = Control.SIZE_EXPAND_FILL; bgold.add_theme_font_size_override("font_size", 16); don_row.add_child(bgold)
	var gsb := StyleBoxFlat.new(); gsb.bg_color = Color("#f0a91e"); gsb.set_corner_radius_all(12); gsb.set_content_margin_all(6)
	for st in ["normal", "hover", "pressed", "focus"]: bgold.add_theme_stylebox_override(st, gsb)
	bgold.add_theme_color_override("font_color", Color.WHITE)
	var bgem := Button.new(); bgem.custom_minimum_size = Vector2(0, 62); bgem.size_flags_horizontal = Control.SIZE_EXPAND_FILL; bgem.add_theme_font_size_override("font_size", 16); don_row.add_child(bgem)
	var esb := StyleBoxFlat.new(); esb.bg_color = Color("#2ea9e0"); esb.set_corner_radius_all(12); esb.set_content_margin_all(6)
	for st in ["normal", "hover", "pressed", "focus"]: bgem.add_theme_stylebox_override(st, esb)
	bgem.add_theme_color_override("font_color", Color.WHITE)
	var refresh := func():
		...
		if lv >= mx:
			eff.text = "%s%s   (МАКС %d/%d)" % [nb.call(lv), _tloc_u(cd), lv, mx]
	...
	bc.add_theme_color_override("font_color", subt)
	bc.pressed.connect(func(): dlg.queue_free()); v.add_child(bc)
		...
		_fb_rest(HTTPClient.METHOD_GET, "/clans/%s/boss" % player_clan, "", func(_c, d):
			...
			if hp <= 0:
				...
				if bstarted != boss_claimed and total > 0 and boss_my_dmg > 0:   # награда один раз с этого босса, по вкладу
					...
					var tok: int = int((round(800.0 * share) + 50) * (1.0 + _cbon("net", "offgold")))   # клан-жетоны ×клан-казна
...
func _prestige_mult() -> float:   # вечно растущий множитель: полином (ранняя игра, баланс сохранён) × exp-хвост (бесконечность на глубине, бьёт стену 1.34^ст)
	...
	return poly * tail
# === МОНЕТИЗАЦИЯ (Фаза А) ===
var diamonds := 50        # 💎 АЛМАЗЫ — премиум (стартовый запас)
...
# 🎯 ПЕРВАЯ ПОКУПКА (спека Дианы): импульс-завлекалка $0.99, разово. Продаёт ФОРУ, не алмазы:
# эпик-оружие первому бойцу + 24ч x2-скорость + бонус алмазов. Триггер после стадии 3, окно 24ч, таймер.
var firstbuy_bought := false
var firstbuy_until := 0.0          # окно оффера истекает в этот _qa_now (0 = ещё не запущен)
const FIRSTBUY_DIAMONDS := 200
const FIRSTBUY_SPEED_SECS := 86400.0   # 24ч x2-скорость
...
var goal_lbl: Label
var event_banner: PanelContainer   # 📅 плашка недельного ивента (LiveOps)
var event_lbl: Label
var _goal_idx := -1           # текущая стадия гол-баннера (для трекинга смены цели)
# === FORCED-FIRST-ACTION туториал (маска с подсветкой, только 1-й запуск) ===
# tut_step: 0=подсветка ПРОКАЧКИ(📊) → 1=ульта → 2=бой → 3=ждём 1-й лут→подсветка ЭКИПИРОВКИ(🦾) → 99=пройден
var tut_step := 0
var _tut_t := 0.0             # таймер текущего шага (для авто-таймаута, не залипать)
var tut_layer: Control        # оверлей-маска (полупрозрачный дим + дырка на цели)
var tut_dim := []             # 4 затемняющих прямоугольника (рамка вокруг дырки)
var tut_ring: Panel           # пульсирующее кольцо-подсветка цели
var tut_cap: Label            # короткая подпись (1 строка)
var tut_skip: Button          # кнопка ПРОПУСТИТЬ
...
# Гача-рейты вынесены в КОНСТАНТЫ — раскрытие шансов (_open_gacha_odds) берёт их же, что и роллер (_gacha_rarity).
# Это гарантирует, что показанные игроку шансы = реальные (требование Google Play / App Store).
const GACHA_P_EPIC := 0.05      # базовый шанс Эпического
const GACHA_P_COMMON := 0.50    # Обычный
const GACHA_P_UNCOMMON := 0.30  # Необычный  (Редкий = остаток = 0.15)
const GACHA_SOFT_PITY := 74     # с этого пулла шанс Эпического начинает расти
const GACHA_HARD_PITY := 90     # на этом пулле — гарантированный Эпический
const GACHA_PITY_STEP := 0.06   # +6% к шансу Эпического за каждый пулл после soft-pity
# === ОФЛАЙН-РЕАКТОР (экспон. сток алмазов): расширяет окно офлайн-дохода ===
var offline_cap_lvl := 0        # купленных уровней реактора
const OFFLINE_CAP_BASE := 43200 # базовое окно офлайн-дохода (12 ч), сек
const OFFLINE_CAP_STEP := 7200  # +2 ч за уровень, сек
const OFFLINE_CAP_MAX := 6      # потолок уровней (+12 ч → до 24 ч окна) — умеренно, не ломает баланс
const OFFLINE_REACTOR_BASE := 150  # цена 1-го уровня в алмазах
const OFFLINE_REACTOR_MULT := 1.5  # рост цены ×1.5 за уровень: 150·225·337·506·759·1139
...
# ENDLESS-режим (бесконечная башня) — ОТДЕЛЬНЫЙ от стадий/престижа прогон волн
var endless_active := false   # сейчас идёт прогон башни (делит боевые переменные с основным режимом → state сохраняется/восстанавливается)
var endless_floor := 0        # текущий этаж в прогоне
var endless_best := 0         # ЛОКАЛЬНЫЙ рекорд (best_floor) — в сейв. TODO: weekly leaderboard через Firebase
var endless_anchor := 1       # best_stage на момент входа (якорь сложности этажей)
var _endless_saved := {}      # снимок основного боевого состояния (stage/sub/in_boss/...) для возврата
...
func _make_item(cls: int, vid: String, rarity: int, slot: String = "module") -> Dictionary:
	...
	# РАСПРЕДЕЛЕНИЕ СТАТОВ ПО РОЛЯМ (Диана): ОРУЖИЕ = урон+офенс (крит/скор), МОДУЛЬ = защита/утилита (HP/заряд). Крит — ТОЛЬКО на оружии (крит = усиление урона), на модулях НЕТ (фидбэк Дианы: «урон и криты на оружие, здоровье на предметах»).
	var others: Array = ["crit", "atk"] if slot == "weapon" else ["hp", "ult"]
...
func _roll_rarity() -> int:
	...
	if _event_is("loot"): lo = min(lo + 1, hi)   # 🎁 Лут-неделя: пол редкости выше на ступень
	if dry_streak >= 6:                 # bad-luck protection: давно без редкого → поднять пол
		lo = min(max(lo, 3), hi)
	if randf() < _cbon("net", "rare") + _cbon("econ", "loot"):   # клан-древа: шанс поднять пол редкости на ступень
		lo = min(lo + 1, hi)
	var pool := []
...
func _recalc_hero(hh: Dictionary) -> void:
	...
	hh["max"] = min(base_hp * rar * aura_hp * aug_hp * (1.0 + _cbon("war", "hp")) * (float(_cfg("surv", 1.0)) if bot else 1.0) * _prestige_mult(), STAT_CAP)   # ×клан-перк HP; float; ×perma-множитель
	# крит / скорость атаки / заряд ульты — от шмоток + аугментов + клан-древо ВОЙНА
	hh["crit"] = clamp(hh["data"]["crit"] + _gear_bonus(hh, "crit") / 100.0 + aug_crit + _cbon("war", "crit"), 0.0, 0.95)   # +клан крит-шанс
	hh["critx"] = hh["data"]["critx"] * aug_critx * (1.0 + _cbon("war", "critx"))   # ×клан крит-урон
	...
	hh["ult_cd_eff"] = hh["data"]["ult_cd"] * aura_ult * max(0.4, 1.0 - _gear_bonus(hh, "ult") / 100.0) * aug_ultcd * max(0.4, 1.0 - _cbon("net", "ultcd"))   # ×клан −КД ульт
...
func _cores_gain() -> int:
	...
	return max(1, int(floor(10.0 * pow(float(depth), 0.75) * aug_core * meta_core * _event_prestige_mult() * (1.0 + _cbon("net", "cores")))))   # ×мета ×ивент ×клан-древо СЕТЬ
...
func _build_nick_prompt() -> void:
	...
	b.pressed.connect(func():
		...
		elif not firstbuy_bought and firstbuy_until == 0.0 and max(best_stage, stage) >= 3:   # 🎯 ПЕРВАЯ ПОКУПКА первой (каскад), после стадии 3, окно 24ч
			firstbuy_until = _qa_now() + FIRSTBUY_WINDOW; _save(); _show_firstbuy_offer()
		elif not starter_bought and not starter_offer_seen and _firstbuy_done() and max(best_stage, stage) >= 3:   # старт-оффер ПОСЛЕ первой покупки (не в 1-ю сессию, не поверх дейлика)
...
func _ready() -> void:
	...
	if not bot and nick != "" and not seen_intro:   # вернувшийся игрок без интро → показать
		...
		_show_daily()
	elif not bot and not firstbuy_bought and firstbuy_until == 0.0 and maxi(best_stage, stage) >= 3:
		# 🎯 ПЕРВАЯ ПОКУПКА первой (каскад) — после стадии 3, окно 24ч
		firstbuy_until = _qa_now() + FIRSTBUY_WINDOW; _save(); _show_firstbuy_offer()
	elif not bot and not starter_bought and not starter_offer_seen and _firstbuy_done() and maxi(best_stage, stage) >= 3:
		# старт-оффер ПОСЛЕ первой покупки (не в 1-ю сессию, не поверх интро/дейлика)
		starter_offer_seen = true; _save(); _show_starter_offer()
...
func _reset() -> void:
	...
	enemies.clear()
	wave = 0
	endless_active = false; endless_floor = 0; endless_best = 0; endless_anchor = 1; _endless_saved = {}
	implants_count = 0
	gold = 0.0
	gold_ps = 2.0
	impl_sel = 0
	...
	cores = 0
	cores_peak = 0.0
	diamonds = 50; x3_unlocked = false; x2_until = 0.0; vip_until = 0.0; starter_bought = false; starter_offer_seen = false; firstbuy_bought = false; firstbuy_until = 0.0; packs_bought = {}; gacha_pity = 0; offline_cap_lvl = 0; last_discovered = ""; ad_boosts = {}; clan_boosts = {}   # 💰 боевая экономика (тест-буст 100k закрыт 10.07 по команде Рамиля)
	...
	_recalc_auras()
	# 🎓 ФОРА ОБУЧЕНИЯ (фидбэк Насти: шаги «апни бойца/тапни ульту» не попадали по таймингу — ресурсы не успевали):
	if tut_step == 0 and not bot:
		gold = 150.0                        # хватает на первую прокачку сразу — шаг «апни бойца» доступен без ожидания
		for hh in heroes: hh["ult_t"] = 2.5   # ульты почти заряжены → шаг «тапни ульту» стабильно срабатывает
	_start_march()
	_refresh_hud()
...
func _party_power() -> float:
	var p := 0.0
	for hh in heroes:
		p += float(hh["dmg"]) * float(hh.get("atk_spd", 1.0)) * float(hh.get("atk_mult", 1.0))
	return min(p, STAT_CAP)   # float: int64 overflow при большом dmg
...
func _fmt_n(n) -> String:
	...
	var s := ""
	if v >= 1.0e15:
		# научная запись ВРУЧНУЮ: GDScript "%" НЕ знает %e (ночной бот-хант: 240k ошибок/2ч, пустые цифры урона на глубине)
		var ex := int(floor(log(v) / log(10.0)))
		s = "%.2fe%d" % [v / pow(10.0, ex), ex]
...
func _show_offline() -> void:
	...
	var earned := _offline_gold   # базовое офлайн-золото уже начислено в gold при загрузке; ×2 доначисляет ещё столько же
	# 📺 ×2 за рекламу — самый дешёвый rewarded-плейсмент (ARPDAU/ретеншн). Реклама=стаб (как _watch_ad_*), реальный SDK на платформе.
	if earned > 0.0:
		var adb := Button.new(); adb.text = _t("offline_ad_x2"); adb.add_theme_font_size_override("font_size", 16); adb.custom_minimum_size = Vector2(0, 50)
		var adsb := StyleBoxFlat.new(); adsb.bg_color = Color("#1f7a3a"); adsb.set_corner_radius_all(10); adsb.border_color = Color("#3ad97a"); adsb.set_border_width_all(2)
		adb.add_theme_stylebox_override("normal", adsb)
		adb.pressed.connect(func():
			_track("ad_impression", {"placement": "offline_x2"})   # KPI: rewarded-просмотр удвоения офлайна
			gold += earned
			_stat_add("gold", earned)
			_stat_add("ads", 1)
			_save(); _refresh_hud()
			_popup_center(_t("offline_ad_pop") % _gsep(earned), Color("#3ad97a"), 2.2)
			panel.queue_free())
		v.add_child(adb)
...
func _load() -> void:
	...
	# tut_step: дефолт 99 для ЛЮБОГО существующего сейва без ключа (уже игравший — туториал не показываем); свежий new game (без файла) → _reset оставит 0
	tut_step = int(d.get("tut_step", 99))
	if best_stage > 30 or rec_prestiges > 0: onboarded = true   # старые сейвы (уже играющие) → онбординг не показываем
	if best_stage > 30 or rec_prestiges > 0: tut_step = 99       # старые сейвы → форсед-туториал не показываем
	...
	if not bot or qa_day_ofs != 0:
		...
		if last_ts > 0:
			...
			if away > 60:
				...
				_offline_gold = min(rate * capped * (1.0 + _cbon("econ", "offline")), STAT_CAP)   # ×клан-древо оффлайн; float-кламп
...
func _spawn_wave() -> void:
	...
	for j in spawn_types.size():
		...
		if boss:
			...
			px = minf(390.0 + j * estep, 600.0 - 62.0 * es)          # кламп с учётом ШИРИНЫ спрайта (широкие типы клипались справа)
		...
		var ey: float = GROUND_Y + 166.0 - ((0.0 if iboss else 96.0 + j * 10.0) if boss else float(j % 2) * 30.0 + j * 4.0)
...
# ===== ENDLESS: спавн волны этажа. Переиспользует боевой пул врагов/арт; сложность масштабируется по этажу.
# eff = «стадия-эквивалент» этажа: якорь (best_stage - OFFSET) + STEP за этаж. Та же HP/DMG-экспонента, что и в стадиях
# (общий баланс) — но без боссов/QTE/лута: чистый прогон «зайди выше». Лёгкий процедурный модификатор для разнообразия.
func _spawn_endless_wave() -> void:
	var eff: float = max(1.0, float(endless_anchor - ENDLESS_ANCHOR_OFFSET) + (endless_floor - 1) * ENDLESS_STEP)
	wave = endless_floor
	var pool := _enemy_pool()
	# процедурный модификатор волны (ротация по этажу): 0=обычная, 1=рой(+враги), 2=броня(+HP), 3=шторм(+скорость)
	var mod := endless_floor % 4
	var count := clampi(2 + int(eff / 5.0), 2, 5)
	if mod == 1: count = clampi(count + 2, 2, 7)   # РОЙ
	var hp_mod := 1.6 if mod == 2 else 1.0          # БРОНЯ
	...
	for j in spawn_types.size():
		...
		if etype == "shield":
			var bub := Polygon2D.new(); bub.polygon = _ellipse_pts(42.0, 52.0); bub.color = Color(0.3, 0.62, 1.0, 0.16)
			bub.position = Vector2(0, -52); bub.z_index = -1; d.add_child(bub)
		# как основная волна: адаптивный шаг + шахматка глубины (не кучей впритык); кламп по ширине спрайта
		var estep: float = clampf(155.0 / float(maxi(spawn_types.size() - 1, 1)), 42.0, 80.0)
		var px: float = minf(390.0 + j * estep, 600.0 - 62.0 * es)
		var ey: float = GROUND_Y + 166.0 - float(j % 2) * 30.0 - j * 4.0
		d.position = Vector2(720, ey); d.z_index = int(ey)
		world.add_child(d)
...
# ВХОД в башню: сохранить основное боевое состояние, обнулить врагов, начать прогон с этажа 1
func _endless_start() -> void:
	if endless_active: return
	if best_stage < ENDLESS_GATE_STAGE:
		_popup_center(_t("endless_locked") % ENDLESS_GATE_STAGE, Color("#9aa0b5"), 2.2)
		return
	_endless_saved = {"stage": stage, "sub": sub, "in_boss": in_boss, "boss_retry": boss_retry, "wave": wave}
	for e in enemies:
		if e["node"]: e["node"].queue_free()
	enemies.clear()
	_qte_clear()
	endless_active = true
	endless_floor = 1
	endless_anchor = best_stage
	in_boss = false; boss_retry = false; sub = 1
	_popup_center(_t("endless_enter"), Color("#7adfff"), 2.4)
	_start_march()
	_refresh_hud()

# КОНЕЦ прогона (отряд пал): награда по этажу + рекорд, возврат основного состояния. Безопасно для основного режима.
func _endless_end() -> void:
	if not endless_active: return
	for e in enemies:   # самоочистка врагов (если вышли через меню в середине боя)
		if e["node"]: e["node"].queue_free()
	enemies.clear()
	_qte_clear()
	var reached := endless_floor
	var record := reached > endless_best
	if record: endless_best = reached
	var reward := reached * ENDLESS_SCRAP_PER_FLOOR
	scrap += reward
	var dia := 0
	if record:
		dia = 5
		diamonds += dia
	# восстановить основной режим
	endless_active = false
	stage = int(_endless_saved.get("stage", stage))
	sub = int(_endless_saved.get("sub", 1))
	in_boss = false
	boss_retry = bool(_endless_saved.get("boss_retry", false))
	_endless_saved = {}
	_save()
	var rec_txt := ("  🏆 %s %d" % [_t("endless_record"), reached]) if record else ""
	_popup_center(_t("endless_result") % [reached, reward] + (("  +%d💎" % dia) if dia > 0 else "") + rec_txt, Color("#ffd24a"), 4.0)
	_start_march()
	_refresh_hud()
...
func _process(delta: float) -> void:
	...
	if not bot: _tut_update(delta)   # forced-first-action туториал (маска с подсветкой)
	...
	if endless_active:
		# ENDLESS: победа → следующий этаж (без стадий/босса/лута); вайп → конец прогона + награда + возврат
		if _all_dead(enemies) and not _all_dead(heroes):
			enemies.clear()
			endless_best = max(endless_best, endless_floor)
			_popup_center(_t("endless_floor_up") % endless_floor, Color("#7ee08a"), 1.4)
			endless_floor += 1
			_start_march()
	...
	if _all_dead(enemies) and not _all_dead(heroes):   # фикс: если 💥взрывной-последний вынес отряд — вайп важнее победы
		...
		if in_boss:
			...
			if ms > milestones_hit:
				milestones_hit = ms
				var msc := int(ms * 300 * (1.0 + _cbon("econ", "scrap")))
				var msd := ms * 4
		...
		_start_march()
	# перф: в активном бою HUD зовётся КАЖДЫЙ кадр (сюда падаем каждый тик fight) — троттлим ~15 Гц; событийные _refresh_hud (покупки/волны/панели) остаются мгновенными
	hud_t -= delta
	if hud_t <= 0.0:
		hud_t = 0.066
		_refresh_hud()
...
func _deal(hh: Dictionary, e: Dictionary, d: float, is_crit := false) -> void:
	...
	if e["hp"] <= 0 and e["alive"]:
		...
		if e.get("type", "") == "bomber":
			...
			var bdmg: float = e["dmg"] * 2.5
			var any_died := false
			for ph in heroes:
				if ph["alive"]:
					ph["hp"] = max(0.0, ph["hp"] - bdmg)
					if ph["hp"] <= 0:
...
func _use_ult(i: int) -> void:
	...
	if tut_step == 1: _tut_advance()   # туториал: тапнул ульту → следующий шаг
	...
	match hh["data"]["ult"]:
		...
		"hack":
			for en in enemies:  # ХАКЕР: плюха по всем
				if en["alive"]:
					_deal(hh, en, hh["dmg"] * 5 * aura_dmg * hack_mult)
...
func _sniper_fire(sn, target) -> void:
	...
	if show_dmg:
		_popup(_fmt_n(d), Color("#00f0ff"), target["node"].position + Vector2(0, -115), 46)   # без эмодзи (шрифт рисовал □)
...
func _qte_marker_hit(entry: Dictionary) -> void:
	...
	if boss != null:
		var sq := 0.0
		for hh in heroes:
			if hh["alive"]: sq += hh["dmg"]
		var d: float = boss["max"] * 0.03 + sq * 2.0
		var att = _first_alive(heroes)
		if att != null: _deal(att, boss, d, true)
		else: boss["hp"] = max(0.0, boss["hp"] - d)
...
const POPUP_MAX := 48           # потолок одновременных попапов: авто-бой x16 × N врагов плодит сотни Label/сек → на моб-вебе просадка; сверх лимита цифры и так сливаются
func _popup(txt: String, col: Color, pos: Vector2, size := 26) -> void:
	if _popup_active >= POPUP_MAX: return   # перегрузка → пропускаем новый попап (визуально не заметно, но снимает GC-давление)
	var l: Label = null
	while not _popup_pool.is_empty():       # берём из пула (пропуская мёртвые ссылки на всякий случай)
		var cand = _popup_pool.pop_back()
		if is_instance_valid(cand): l = cand; break
	if l == null:
		l = Label.new()
		l.z_index = 50
		world.add_child(l)
...
func _refresh_hud() -> void:
	...
	if endless_active:
		wave_label.text = "🗼 %s %d   🏆%d" % [_t("endless_floor"), endless_floor, endless_best] + ("   ⚔" if phase == "fight" else "   ▶")
	else:
	...
	if boss_btn:
		boss_btn.visible = boss_retry and not in_boss and not endless_active   # кнопка только для ретрая (свежий заход = авто)
...
# попап описания недельного ивента (тап по баннеру 📅)
func _open_event_popup() -> void:
	var loc: Dictionary = EVENT_LOC[_weekly_event()["id"]]
	var panel := Control.new(); panel.set_anchors_preset(Control.PRESET_FULL_RECT); panel.z_index = 3500; hud.add_child(panel)
	var dim := ColorRect.new(); dim.color = Color(0, 0, 0, 0.7); dim.set_anchors_preset(Control.PRESET_FULL_RECT)
	dim.gui_input.connect(func(ev): if ev is InputEventMouseButton and ev.pressed: panel.queue_free())
	panel.add_child(dim)
	var card := PanelContainer.new()
	var sb := StyleBoxFlat.new(); sb.bg_color = Color(0.09, 0.06, 0.03, 0.99); sb.set_corner_radius_all(14); sb.border_color = Color("#ffb02e"); sb.set_border_width_all(2); sb.set_content_margin_all(18)
	card.add_theme_stylebox_override("panel", sb); card.position = Vector2(W * 0.5 - 200, 230); card.custom_minimum_size = Vector2(400, 0)
	panel.add_child(card)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 12); card.add_child(v)
	v.add_child(_lbl(_t("we_banner"), 14, Color("#ffb02e"), HORIZONTAL_ALIGNMENT_CENTER))
	v.add_child(_lbl(_tloc(loc, "name"), 22, Color("#ffe1a8"), HORIZONTAL_ALIGNMENT_CENTER))
	var d := _lbl(_tloc(loc, "desc"), 15, Color("#cfe8ff"), HORIZONTAL_ALIGNMENT_CENTER)
...
func _build() -> void:
	...
	# 📅 БАННЕР НЕДЕЛЬНОГО ИВЕНТА — компактная плашка вверху-справа, тап → попап (не навязчиво)
	...
	event_lbl = Label.new()
	event_lbl.add_theme_font_size_override("font_size", 13)
	event_lbl.add_theme_color_override("font_color", Color("#ffe1a8"))
	event_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	event_banner.add_child(event_lbl)
	_build_tut()

# === ОВЕРЛЕЙ ТУТОРИАЛА: маска-дим с «дыркой» на цели + кольцо + подпись + ПРОПУСТИТЬ ===
func _build_tut() -> void:
	tut_layer = Control.new()
	tut_layer.set_anchors_preset(Control.PRESET_FULL_RECT)
	tut_layer.mouse_filter = Control.MOUSE_FILTER_IGNORE   # сам слой прозрачен для кликов, блокируют дим-рамки
	tut_layer.z_index = 1500   # выше HUD/гол-баннера(500), ниже всех панелей/диалогов(2000+) → открытая панель перекрывает
	tut_layer.visible = false
	hud.add_child(tut_layer)
	tut_dim.clear()
	for i in 4:   # 4 затемняющих прямоугольника = рамка вокруг прозрачной дырки
		var r := ColorRect.new()
		r.color = Color(0, 0, 0, 0.72)
		r.mouse_filter = Control.MOUSE_FILTER_STOP   # блок тапов мимо подсвеченной кнопки (forced action)
		tut_layer.add_child(r)
		tut_dim.append(r)
	tut_ring = Panel.new()   # пульсирующее неоновое кольцо вокруг цели
	var rsb := StyleBoxFlat.new()
	rsb.bg_color = Color(0, 0, 0, 0); rsb.set_corner_radius_all(12)
	rsb.border_color = Color("#00f0ff"); rsb.set_border_width_all(3)
	tut_ring.add_theme_stylebox_override("panel", rsb)
	tut_ring.mouse_filter = Control.MOUSE_FILTER_IGNORE   # кольцо не перехватывает тап по цели
	tut_layer.add_child(tut_ring)
	tut_cap = Label.new()
	tut_cap.add_theme_font_size_override("font_size", 19)
	tut_cap.add_theme_color_override("font_color", Color("#cfe8ff"))
	tut_cap.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.9))
	tut_cap.add_theme_constant_override("outline_size", 6)
	tut_cap.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	tut_cap.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	tut_cap.mouse_filter = Control.MOUSE_FILTER_IGNORE
	tut_cap.custom_minimum_size = Vector2(420, 0)
	tut_cap.size = Vector2(420, 0)
	tut_layer.add_child(tut_cap)
	tut_skip = Button.new()   # ПРОПУСТИТЬ — закрыть туториал навсегда
	tut_skip.text = _t("tut_skip")
	tut_skip.add_theme_font_size_override("font_size", 14)
	tut_skip.position = Vector2(W - 132, 10)
	tut_skip.custom_minimum_size = Vector2(122, 32)
	tut_skip.pressed.connect(_tut_finish)
	tut_layer.add_child(tut_skip)

# спрятать туториал под открытой панелью/диалогом (любой высокий модальный слой)
func _tut_blocked() -> bool:
	if inv_open or impl_open: return true
	if reboot_panel and reboot_panel.visible: return true
	if nick_panel and nick_panel.visible: return true
	for c in hud.get_children():
		if c == tut_layer: continue
		if c is Control and c.visible and c.z_index >= 1800:
			return true
	return false

# объединённый прямоугольник бара ульт (цель шага 2)
func _ults_rect() -> Rect2:
	if hero_ults.is_empty(): return Rect2(36, H - 150, W - 72, 84)
	var r: Rect2 = hero_ults[0].get_global_rect()
	for i in range(1, hero_ults.size()):
		r = r.merge(hero_ults[i].get_global_rect())
	return r

# разместить дим-рамку с «дыркой» на hole + подпись
func _tut_hole(hole: Rect2, caption: String) -> void:
	var pad := 6.0
	var hx: float = hole.position.x - pad
	var hy: float = hole.position.y - pad
	var hw: float = hole.size.x + pad * 2.0
	var hh2: float = hole.size.y + pad * 2.0
	tut_dim[0].position = Vector2(0, 0); tut_dim[0].size = Vector2(W, max(0.0, hy))                                  # сверху
	tut_dim[1].position = Vector2(0, hy + hh2); tut_dim[1].size = Vector2(W, max(0.0, H - (hy + hh2)))               # снизу
	tut_dim[2].position = Vector2(0, hy); tut_dim[2].size = Vector2(max(0.0, hx), hh2)                               # слева
	tut_dim[3].position = Vector2(hx + hw, hy); tut_dim[3].size = Vector2(max(0.0, W - (hx + hw)), hh2)              # справа
	tut_ring.position = Vector2(hx, hy); tut_ring.size = Vector2(hw, hh2); tut_ring.visible = true
	tut_cap.text = caption
	var cy: float = hy - 52.0   # подпись над дыркой; если у верха — под дыркой
	if cy < 84.0: cy = hy + hh2 + 12.0
	tut_cap.position = Vector2(W * 0.5 - 210.0, cy)

# каждый кадр: вести игрока по шагам, авто-таймаут чтобы не залипать
func _tut_update(delta: float) -> void:
	if not tut_layer: return
	if bot or tut_step >= 99 or nick == "":
		tut_layer.visible = false
		return
	_tut_t += delta
	if _tut_blocked():   # панель/диалог открыт — туториал ждёт под ним
		tut_layer.visible = false
		return
	var cap := ""
	var hole := Rect2()
	var show := true
	match tut_step:
		0:   # подсветка ПРОКАЧКИ (📊) — ждём первый апгрейд (advance в _upgrade_level)
			hole = inv_btn.get_global_rect(); cap = _t("tut_s1")
			if _tut_t > 30.0: _tut_advance()   # таймаут: не залипать если игрок не тапнул
		1:   # подсветка бара ульт — ждём тап ульты (advance в _use_ult), мягкий таймаут
			hole = _ults_rect(); cap = _t("tut_s2")
			if _tut_t > 14.0: _tut_advance()   # ульта может быть не готова — не блокируем жёстко
		2:   # подсветка поля боя — короткая пауза
			hole = Rect2(36, 150, W - 72, 470); cap = _t("tut_s3")
			if _tut_t > 6.0: _tut_advance()
		3:   # ждём 1-й лут; выпал → подсветка ЭКИПИРОВКИ (🦾) + вау-подпись (advance в _toggle_impl)
			if new_gear.size() > 0:
				hole = impl_btn.get_global_rect(); cap = _t("tut_s4")
			else:
				show = false
				if _tut_t > 50.0: _tut_finish()   # лут долго не падает — завершить, не держать дим
	if not show:
		tut_layer.visible = false
		return
	tut_layer.visible = true
	_tut_hole(hole, cap)
	tut_ring.modulate.a = 0.55 + 0.45 * sin(Time.get_ticks_msec() / 220.0)   # пульс кольца

func _tut_advance() -> void:
	tut_step += 1
	_tut_t = 0.0
	if tut_step >= 4: tut_step = 99
	_track("tutorial_step", {"step": "tut_%d" % tut_step})
	_save()

func _tut_finish() -> void:   # скип/завершение — больше не показывать
	tut_step = 99
	_tut_t = 0.0
...
func _iap_grant(pid: String, token: String, acked: bool) -> void:
	...
	if IAP_DIAMONDS.has(pid):
		if fresh:
			var got := _pack_grant_amount(int(IAP_DIAMONDS[pid]))   # ×2 первая покупка пака
			diamonds += got
			_track("iap_real", {"item": pid, "granted": got})
			_popup_center(_t("shop_buy_pop") % got, Color("#ffd24a"), 1.6)
		...
		if fresh and not firstbuy_bought:
			_firstbuy_grant()
			_track("iap_real", {"item": pid})
		if not acked: iap.acknowledge_purchase(token)   # разовый non-consumable → acknowledge
	elif pid == "event_pack":
		if fresh:
			diamonds += 1000
...
func _iap_buy_diamonds(pid: String, amt: int, price: String) -> void:
	if iap != null and iap_ready:
		...
		return
	var got := _pack_grant_amount(amt)   # ×2 первая покупка пака (стаб web/тест)
	diamonds += got
	_track("iap_purchase", {"item": pid, "amount": got, "price": price})
	_save(); _refresh_hud()
	_popup_center(_t("shop_buy_pop") % got, Color("#ffd24a"), 1.6)
...
# === 🎯 ПЕРВАЯ ПОКУПКА — импульс-оффер $0.99 (форой: эпик-оружие + 24ч x2 + алмазы) ===
func _firstbuy_active() -> bool:
	return not firstbuy_bought and firstbuy_until > 0.0 and _qa_now() < firstbuy_until
func _firstbuy_done() -> bool:
	return firstbuy_bought or (firstbuy_until > 0.0 and _qa_now() >= firstbuy_until)

# выдать эпик-оружие первому бойцу отряда
func _grant_first_weapon() -> void:
	if heroes.is_empty(): return
	var hh = heroes[0]
	var variants := _slot_variants("weapon", int(hh["cls"]))
	if variants.is_empty(): return
	var vv = variants[randi() % variants.size()]
	var it := _make_item(int(hh["cls"]), str(vv["id"]), 4, "weapon")   # эпик (rar 4)
	it["lvl"] = max(1, stage)
	var key := _ik(str(vv["id"]), 4)
	var g = hh["gear"]["weapon"]
	if not g.has(key) or _item_power(it) > _item_power(g[key]):
		g[key] = it
		new_gear["0:weapon"] = int(new_gear.get("0:weapon", 0)) + 1
	_recalc_hero(hh)

# выдача набора (общая для стаба и реального биллинга)
func _firstbuy_grant() -> void:
	if firstbuy_bought: return
	firstbuy_bought = true
	diamonds += FIRSTBUY_DIAMONDS
	x2_until = max(x2_until, _qa_now()) + FIRSTBUY_SPEED_SECS   # 24ч x2-скорость (продлевает если активна)
	_grant_first_weapon()
	if not bot: _set_speed(2.0)
	_save(); _refresh_hud()
	_popup_center(_t("firstbuy_pop"), Color("#ffcf3a"), 2.6)

func _buy_firstbuy() -> void:
	if firstbuy_bought: return
	if iap != null and iap_ready:
		iap.purchase("first_buy")   # грант придёт в on_purchase_updated
		return
	_firstbuy_grant()   # стаб (web/тест)
	_track("iap_purchase", {"item": "first_buy", "price": "0.99$"})

func _show_firstbuy_offer() -> void:
	var panel := Control.new(); panel.set_anchors_preset(Control.PRESET_FULL_RECT); panel.z_index = 3320; hud.add_child(panel)
	var dim := ColorRect.new(); dim.color = Color(0, 0, 0, 0.8); dim.set_anchors_preset(Control.PRESET_FULL_RECT)
	dim.gui_input.connect(func(ev): if ev is InputEventMouseButton and ev.pressed: panel.queue_free())
	panel.add_child(dim)
	var card := PanelContainer.new()
	var sb := StyleBoxFlat.new(); sb.bg_color = Color(0.14, 0.11, 0.03, 0.99); sb.set_corner_radius_all(14); sb.border_color = Color("#ffcf3a"); sb.set_border_width_all(3); sb.set_content_margin_all(20)
	card.add_theme_stylebox_override("panel", sb); card.position = Vector2(W * 0.5 - 200, 300); card.custom_minimum_size = Vector2(400, 0)
	panel.add_child(card)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 12); card.add_child(v)
	v.add_child(_lbl(_t("firstbuy_offer_t"), 22, Color("#ffe089"), HORIZONTAL_ALIGNMENT_CENTER))
	v.add_child(_lbl(_t("firstbuy_offer_sub"), 12, Color("#e0b85a"), HORIZONTAL_ALIGNMENT_CENTER))
	v.add_child(_lbl(_t("firstbuy_perks") % FIRSTBUY_DIAMONDS, 15, Color("#cfe6ff"), HORIZONTAL_ALIGNMENT_CENTER))
	var buy := _prim_btn(_t("firstbuy_btn"), Color("#c9971e"), 54, 17); v.add_child(buy)
	buy.pressed.connect(func(): _buy_firstbuy(); panel.queue_free())
	var later := Button.new(); later.text = _t("firstbuy_later"); later.custom_minimum_size = Vector2(0, 38); later.add_theme_font_size_override("font_size", 13); later.add_theme_color_override("font_color", Color("#9aa0b5"))
	later.pressed.connect(func(): panel.queue_free()); v.add_child(later)
...
# 💎 ×2 на ПЕРВУЮ покупку каждого пака: возвращает выданное кол-во и помечает пак купленным
func _pack_grant_amount(amt: int) -> int:
	if not bool(packs_bought.get(str(amt), false)):
		packs_bought[str(amt)] = true
		return amt * 2
	return amt
...
func _open_shop() -> void:
	...
	panel.add_child(dim)
	var card := PanelContainer.new()
	card.add_theme_stylebox_override("panel", _grey_style(Color("#ffd24a"))); card.custom_minimum_size = Vector2(430, 0)
	var crow := HBoxContainer.new(); crow.alignment = BoxContainer.ALIGNMENT_CENTER; crow.position = Vector2(0, 110); crow.size = Vector2(W, 0); crow.add_child(card); panel.add_child(crow)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 10); card.add_child(v)
	v.add_child(_lbl(_t("shop_title") % diamonds, 22, Color("#ffe089"), HORIZONTAL_ALIGNMENT_CENTER))
	# 🗂 ВКЛАДКИ
	var tabs := HBoxContainer.new(); tabs.add_theme_constant_override("separation", 5); tabs.size_flags_horizontal = Control.SIZE_EXPAND_FILL; v.add_child(tabs)
	var _mktab := func(id: String, label: String, accent: Color) -> void:
		var tb := Button.new(); tb.text = label; tb.custom_minimum_size = Vector2(0, 38); tb.size_flags_horizontal = Control.SIZE_EXPAND_FILL; tb.add_theme_font_size_override("font_size", 12)
		var active: bool = shop_tab == id
		var sbx := StyleBoxFlat.new(); sbx.bg_color = accent if active else Color(0.13, 0.15, 0.20); sbx.set_corner_radius_all(9); sbx.set_content_margin_all(3)
		for st in ["normal", "hover", "pressed", "focus"]: tb.add_theme_stylebox_override(st, sbx)
...
# 🎯 баннер первой покупки с таймером (переиспользуется)
func _shop_firstbuy_banner(v: VBoxContainer, panel: Control) -> void:
	var fb := PanelContainer.new()
	var fsb := StyleBoxFlat.new(); fsb.bg_color = Color(0.16, 0.13, 0.03); fsb.set_corner_radius_all(12); fsb.border_color = Color("#ffcf3a"); fsb.set_border_width_all(3); fsb.set_content_margin_all(10)
	fb.add_theme_stylebox_override("panel", fsb); v.add_child(fb)
	var fv := VBoxContainer.new(); fv.add_theme_constant_override("separation", 5); fb.add_child(fv)
	fv.add_child(_lbl(_t("firstbuy_tile") + " — 0.99$", 19, Color("#ffe089"), HORIZONTAL_ALIGNMENT_CENTER))
	fv.add_child(_lbl(_t("firstbuy_perks") % FIRSTBUY_DIAMONDS, 12, Color("#cfe6ff"), HORIZONTAL_ALIGNMENT_CENTER))
	var ft := _lbl("", 14, Color("#ff9a3c"), HORIZONTAL_ALIGNMENT_CENTER); fv.add_child(ft)
	var fbuy := _prim_btn(_t("firstbuy_btn"), Color("#c9971e"), 46, 16); fv.add_child(fbuy)
	fbuy.pressed.connect(func(): _buy_firstbuy(); panel.queue_free())
	var fupd := func():
		var left := int(max(0.0, firstbuy_until - _qa_now()))
		ft.text = _t("firstbuy_timer") % ("%02d:%02d:%02d" % [left / 3600, (left % 3600) / 60, left % 60])
	fupd.call()
	var ftmr := Timer.new(); ftmr.wait_time = 1.0; ftmr.autostart = true; fb.add_child(ftmr)
	ftmr.timeout.connect(func(): if is_instance_valid(ft): fupd.call())

# 🎁 СПЕЦ — премиум-баннеры (первая покупка, старт, VIP, ивент, оффлайн-реактор)
...
func _shop_tab_resources(v: VBoxContainer, panel: Control) -> void:
	...
	for pi in _packs.size():
		...
		# (подписи «≈N круток» откатили — фидбэк Дианы: не привязывать валюту к 1 сценарию; классический магазин валюты)
...
func _reactor_cost() -> int:
	return int(round(OFFLINE_REACTOR_BASE * pow(OFFLINE_REACTOR_MULT, offline_cap_lvl)))

func _reactor_hours() -> int:
	return (OFFLINE_CAP_BASE + offline_cap_lvl * OFFLINE_CAP_STEP) / 3600

func _buy_reactor() -> bool:
	if offline_cap_lvl >= OFFLINE_CAP_MAX: return false
	var cost := _reactor_cost()
	if diamonds < cost: return false
	diamonds -= cost; offline_cap_lvl += 1
	_track("buy_reactor", {"lvl": offline_cap_lvl, "cost": cost})
	_save(); _refresh_hud(); _popup_center(_t("reactor_pop"), Color("#7adfff"), 1.8)
	return true

func _open_offline_reactor() -> void:
	var panel := Control.new(); panel.set_anchors_preset(Control.PRESET_FULL_RECT); panel.z_index = 3450; hud.add_child(panel)
	var dim := ColorRect.new(); dim.color = Color(0, 0, 0, 0.7); dim.set_anchors_preset(Control.PRESET_FULL_RECT)
	dim.gui_input.connect(func(ev): if ev is InputEventMouseButton and ev.pressed: panel.queue_free())
	panel.add_child(dim)
	var card := PanelContainer.new()
	var sb := StyleBoxFlat.new(); sb.bg_color = Color(0.04, 0.09, 0.13, 0.99); sb.set_corner_radius_all(14); sb.border_color = Color("#7adfff"); sb.set_border_width_all(2); sb.set_content_margin_all(18)
	card.add_theme_stylebox_override("panel", sb); card.position = Vector2(W * 0.5 - 205, 150); card.custom_minimum_size = Vector2(410, 0)
	panel.add_child(card)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 10); card.add_child(v)
	v.add_child(_lbl(_t("reactor_title"), 19, Color("#7adfff"), HORIZONTAL_ALIGNMENT_CENTER))
	var dl := _lbl(_t("reactor_desc"), 12, Color("#cfe6ff"), HORIZONTAL_ALIGNMENT_CENTER); dl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; dl.custom_minimum_size = Vector2(370, 0); v.add_child(dl)
	var cur := _lbl(_t("reactor_cur") % [_reactor_hours(), offline_cap_lvl, OFFLINE_CAP_MAX], 14, Color("#ffd24a"), HORIZONTAL_ALIGNMENT_CENTER); v.add_child(cur)
	var buy := Button.new(); buy.custom_minimum_size = Vector2(0, 48); buy.add_theme_font_size_override("font_size", 16)
	var _upd := func():
		if offline_cap_lvl >= OFFLINE_CAP_MAX:
			buy.text = _t("reactor_max") % _reactor_hours(); buy.disabled = true
		else:
			buy.text = _t("reactor_buy") % _reactor_cost(); buy.disabled = diamonds < _reactor_cost()
		cur.text = _t("reactor_cur") % [_reactor_hours(), offline_cap_lvl, OFFLINE_CAP_MAX]
	_upd.call()
	buy.pressed.connect(func(): if _buy_reactor(): _upd.call())
	v.add_child(buy)
	var bc := Button.new(); bc.text = _t("close_x"); bc.custom_minimum_size = Vector2(0, 40); bc.pressed.connect(func(): panel.queue_free()); v.add_child(bc)
...
func _gacha_rarity() -> int:
	gacha_pity += 1
	if gacha_pity >= GACHA_HARD_PITY:        # хард-pity: гарант Эпического
		gacha_pity = 0; return 4
	var p4 := GACHA_P_EPIC                    # софт-pity: с GACHA_SOFT_PITY пулла шанс Эпического растёт
	if gacha_pity >= GACHA_SOFT_PITY: p4 = GACHA_P_EPIC + (gacha_pity - (GACHA_SOFT_PITY - 1)) * GACHA_PITY_STEP
	# ОДИН roll → точные объявленные шансы 50/30/15/5 (баг-хант R3: раньше редкий выходил 19% вместо 15%)
	var r := randf()
	if r < p4: gacha_pity = 0; return 4                              # Эпический (+pity-рампа)
	if r < p4 + GACHA_P_COMMON: return 1                             # Обычный
	if r < p4 + GACHA_P_COMMON + GACHA_P_UNCOMMON: return 2          # Необычный
	return 3                                                         # Редкий (остаток)
...
func _open_gacha_odds() -> void:
	var panel := Control.new(); panel.set_anchors_preset(Control.PRESET_FULL_RECT); panel.z_index = 3500; hud.add_child(panel)
	var dim := ColorRect.new(); dim.color = Color(0, 0, 0, 0.75); dim.set_anchors_preset(Control.PRESET_FULL_RECT)
	dim.gui_input.connect(func(ev): if ev is InputEventMouseButton and ev.pressed: panel.queue_free())
	panel.add_child(dim)
	var card := PanelContainer.new()
	var sb := StyleBoxFlat.new(); sb.bg_color = Color(0.10, 0.06, 0.12, 0.99); sb.set_corner_radius_all(14); sb.border_color = Color("#ffd24a"); sb.set_border_width_all(2); sb.set_content_margin_all(18)
	card.add_theme_stylebox_override("panel", sb); card.position = Vector2(W * 0.5 - 215, 90); card.custom_minimum_size = Vector2(430, 0)
	panel.add_child(card)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 7); card.add_child(v)
	v.add_child(_lbl(_t("gacha_odds_title"), 19, Color("#ffd24a"), HORIZONTAL_ALIGNMENT_CENTER))
	# таблица базовых шансов (Редкий = остаток до 100%)
	var p_rare: float = 1.0 - GACHA_P_EPIC - GACHA_P_COMMON - GACHA_P_UNCOMMON
	v.add_child(_lbl(_t("gacha_odds_rates_h"), 13, Color("#d9c7ff"), HORIZONTAL_ALIGNMENT_CENTER))
	var rates := "• %s — %.1f%%\n• %s — %.1f%%\n• %s — %.1f%%\n• %s — %.1f%%" % [
		_rarity_name(1), GACHA_P_COMMON * 100.0,
		_rarity_name(2), GACHA_P_UNCOMMON * 100.0,
		_rarity_name(3), p_rare * 100.0,
		_rarity_name(4), GACHA_P_EPIC * 100.0]
	v.add_child(_lbl(rates, 14, Color.WHITE, HORIZONTAL_ALIGNMENT_CENTER))
	# pity-гарант
	v.add_child(_lbl(_t("gacha_odds_pity_h"), 13, Color("#ff7adf"), HORIZONTAL_ALIGNMENT_CENTER))
	var pity := _t("gacha_odds_pity_b") % [GACHA_SOFT_PITY, int(GACHA_PITY_STEP * 100.0), GACHA_HARD_PITY]
	var pl := _lbl(pity, 12, Color("#cfe6ff"), HORIZONTAL_ALIGNMENT_LEFT); pl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; pl.custom_minimum_size = Vector2(390, 0); v.add_child(pl)
	# цена + дисклеймер
	v.add_child(_lbl(_t("gacha_odds_cost") % [_event_gacha_cost(GACHA_COST1), _event_gacha_cost(GACHA_COST10)], 12, Color("#9a8fb5"), HORIZONTAL_ALIGNMENT_CENTER))
	var fl := _lbl(_t("gacha_odds_foot"), 11, Color("#7a7090"), HORIZONTAL_ALIGNMENT_CENTER); fl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; fl.custom_minimum_size = Vector2(390, 0); v.add_child(fl)
	var bc := Button.new(); bc.text = _t("close_x"); bc.custom_minimum_size = Vector2(0, 40); bc.pressed.connect(func(): panel.queue_free()); v.add_child(bc)
...
func _open_more() -> void:
	...
	elif best_stage >= ENDLESS_GATE_STAGE:
...
func _upgrade_level(i: int) -> void:
	...
	if buy_mult == 0:
		...
		var bought := 0
		while gold >= _lc(hh) and bought < 1000000:   # +страховка от зависания (баг-ревью)
			gold -= _lc(hh)
			hh["level"] += 1
			hh["lvl_cost"] = int(min(hh["lvl_cost"] * LVL_COST_GROWTH + 2.0, 9.0e18))   # кламп: без него int64-overflow на ур.~352 → цена в минус → фриз/бесконечное золото (баг-ревью крит)
			bought += 1
		if bought > 0:
			_recalc_hero(hh)
			_refresh_inv()
			if tut_step == 0: _tut_advance()   # туториал: первый апгрейд сделан → следующий шаг
		return
	...
	for k in n:
		gold -= _lc(hh)
		hh["level"] += 1
		hh["lvl_cost"] = int(min(hh["lvl_cost"] * LVL_COST_GROWTH + 2.0, 9.0e18))   # кламп overflow (баг-ревью крит)
	_recalc_hero(hh)
	_refresh_inv()
	if tut_step == 0: _tut_advance()   # туториал: первый апгрейд сделан → следующий шаг
...
func _affordable_levels(hh: Dictionary) -> int:
	...
	while g >= cost and n < 100000:
		g -= cost
		cost = int(min(cost * LVL_COST_GROWTH + 2.0, 9.0e18))   # кламп overflow (баг-ревью крит)
		n += 1
	return n

func _lc(hh: Dictionary) -> int:   # цена уровня со скидкой клан-древа СИНДИКАТ
	return int(hh["lvl_cost"] * (1.0 - _cbon("econ", "upg")))

func _batch_cost(hh: Dictionary, n: int) -> int:
	# суммарная стоимость следующих n уровней (цена растёт ×1.09+2 за уровень)
	var cost: int = hh["lvl_cost"]
	var disc := 1.0 - _cbon("econ", "upg")
	var total := 0
	for k in n:
		total += int(cost * disc)
		cost = int(min(cost * LVL_COST_GROWTH + 2.0, 9.0e18))   # кламп overflow (баг-ревью крит)
...
func _toggle_impl() -> void:
	...
	if impl_open:
		if tut_step == 3: _tut_finish()   # туториал: открыл экипировку (надеть 1-й лут) → завершён
		_refresh_impl()
...
func _gear_upgrade_cost(inst: Dictionary) -> int:
	return int(min(20.0 * pow(1.5, int(inst.get("up", 0))), 9.0e18))   # кламп overflow (баг-ревью: тот же класс, что lvl_cost)
...
func _gem_card(texname: String, emoji: String, amount_txt: String, badge_txt: String, badge_col: Color, buy_txt: String, buy_col: Color, cb: Callable, value_txt := "") -> Control:
	var card := PanelContainer.new(); card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var sb := StyleBoxFlat.new(); sb.bg_color = Color(0.15, 0.17, 0.22); sb.set_corner_radius_all(16); sb.set_content_margin_all(10)
	card.add_theme_stylebox_override("panel", sb)
	var v := VBoxContainer.new(); v.add_theme_constant_override("separation", 3); card.add_child(v)
	var tex := _shop_tex(texname)
	if tex != null:   # иллюстрация крупная — главный герой карточки (~50% высоты)
		var tr := TextureRect.new(); tr.texture = tex; tr.expand_mode = TextureRect.EXPAND_IGNORE_SIZE; tr.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
		tr.custom_minimum_size = Vector2(0, 92); tr.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		v.add_child(tr)
	else:
		v.add_child(_lbl(emoji, 44, badge_col.lightened(0.2), HORIZONTAL_ALIGNMENT_CENTER))
	v.add_child(_lbl(amount_txt, 22, Color.WHITE, HORIZONTAL_ALIGNMENT_CENTER))   # крупное кол-во — первое что видит игрок
	if value_txt != "":   # 🎰 ЦЕННОСТЬ в КРУТКАХ (спека Дианы: продаём результат, а не цифру)
		v.add_child(_lbl(value_txt, 12, Color("#9fb0d0"), HORIZONTAL_ALIGNMENT_CENTER))
	if badge_txt != "":
		v.add_child(_lbl(badge_txt, 11, badge_col, HORIZONTAL_ALIGNMENT_CENTER))   # бейдж ×2/выгода
	var buy := _prim_btn(buy_txt, buy_col, 36, 15); v.add_child(buy)                # кнопка покупки (ниже/спокойнее)
	buy.pressed.connect(cb)
	return card

# 🎰 перевод алмазов в КРУТКИ (спека Дианы: игрок мыслит призывами, а не числом)
func _pulls_label(amt: int) -> String:
	if amt >= 4000:   return _t("shop_val_guar") % (amt / GACHA_COST10)
	elif amt >= 1500: return _t("shop_val_x10") % (amt / GACHA_COST10)
	else:             return _t("shop_val_single") % max(1, amt / GACHA_COST1)
