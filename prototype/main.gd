...
const FORMATION := [
	{"x": 48.0,  "y": 92.0,  "s": 0.82},   # СНАЙПЕР — тыл (дальний-лево)
	{"x": 170.0, "y": 70.0,  "s": 0.90},   # ШТУРМОВИК — центр-верх (глубже)
	{"x": 262.0, "y": 128.0, "s": 1.00},   # ТАНК — остриё/фронт (ближе к врагам, крупный)
	{"x": 110.0, "y": 156.0, "s": 0.88},   # ХАКЕР — низ-перед. Формация опущена к ЦЕНТРУ дороги (+48, фидбэк Рамиля «слишком вверх смещено»)
]
...
const VERSION := "1.9.55" # версия билда (показывается в игре: тестер видит совпадает ли с последней → надо ли обновиться). Бампить КАЖДЫЙ деплой.
var nick := ""
...
func _spawn_wave() -> void:
	...
	for j in spawn_types.size():
		...
		var px: float
		if boss:
			px = 420.0 if iboss else min(448.0 + j * 26.0, 526.0)   # босс-центр, свита плотнее (кламп 526 — не прижимать к краю, «болванчик в жопе» фидбэк Рамиля)
		else:
			px = min(352.0 + j * 48.0, 545.0)                        # рядок врагов — шире растянут по дороге, крайний ≤545
		var ey: float = GROUND_Y + 110.0 - ((0.0 if iboss else 20.0 + j * 12.0) if boss else j * 20.0)   # опущено к центру дороги (+48, фидбэк Рамиля)
...
func _spawn_endless_wave() -> void:
	...
	for j in spawn_types.size():
		...
		var ey: float = GROUND_Y + 110.0 - j * 20.0   # опущено к центру дороги (как основная волна)
		d.position = Vector2(720, ey); d.z_index = int(ey)
		world.add_child(d)
