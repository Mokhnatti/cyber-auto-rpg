...
const VERSION := "0.8.5"   # версия билда (показывается в игре: тестер видит совпадает ли с последней → надо ли обновиться). Бампить КАЖДЫЙ деплой.
var nick := ""
var tele_t := 30.0
